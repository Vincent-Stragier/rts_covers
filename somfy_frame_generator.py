...
def write_config_file(content: dict, path: str = "") -> None:
    ...
    return json.dump(content, open(path, "w"), indent=4)


def save_counter(path: str = "", counter: int = 0) -> None:
    """Save a counter in a file"""
    try:
        os.makedirs(os.path.dirname(path))
    except FileExistsError:
        pass
    open(path, "w+").write(str(counter % 2 ** 16))
...
def generate_somfy_base_frame(command, rolling_code_counter, remote_ID) -> list:
...
def generate_somfy_add_frame_checksum(frame):
...
def generate_somfy_obfuscate_frame(frame):
    """Obfuscate a frame."""
...
def generate_somfy_full_frame(command, rolling_code_counter, remote_ID) -> list:
...
def counters_path(config_file):
    """Return the path to the counters directory"""
    config = read_config_file(config_file)
    counters_root = config["counters_path"]
    if not os.path.isabs(config["counters_path"]):
        counters_root = os.path.join(
            os.path.dirname(config_file), counters_root.replace("./", "")
        )
    return counters_root


def shutter_id_and_counter(config_file, shutter_key) -> tuple:
    ...
    shutter_id = config["shutters"][shutter_key]["id"]
    counter_path = os.path.join(counters_root, f"{shutter_id}.txt")
    return int(shutter_id, 16), read_counter(counter_path)


def increment_shutter_counter(config_file, shutter_key):
    ...
    counter_path = os.path.join(counters_root, f"{shutter_id}.txt")
    save_counter(
        path=counter_path, counter=read_counter(path=counter_path) + 1
    )


def decrement_shutter_counter(config_file, shutter_key):
...
if __name__ == "__main__":
    ...
    # frame = generate_somfy_base_frame(
    #     command=COMMANDS["UP"],
    ...
    # frame = generate_somfy_add_frame_checksum(frame)
    # print_frame(frame)

    # frame = generate_somfy_obfuscate_frame(frame)
    # print_frame(frame)
