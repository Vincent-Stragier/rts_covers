...
from somfy_frame_generator import (
    generate_somfy_full_frame, frame_to_string, shutter_id_and_counter,
    read_config_file, str_to_int
)
...
def decode_json_commands(settings: str, recipe: str):
    ...
    for shutter_command in read_config_file(settings)["Recipes"][recipe]:
        ...
        commands.append({
            ...
            "frame": frame_to_string(generate_somfy_full_frame(
                shutter_command["command"],
                counter,
                shutter_id
            ))})
    return commands
        ...
        if command.startswith("send(") and command.endswith(")"):
            ...
            if len(arguments) in (2, 3):
                ...
                decoded_commands.append(
                    {"command_base": "send", "arguments": arguments,
                     "shutter_id": shutter_id, "command": arguments[1],
                     "counter": counter, "frame": frame_to_string(frame),
                     "shutter": arguments[0]})
            else:
                raise ValueError(
